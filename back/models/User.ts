...
const UserSchema = new mongoose.Schema<HydratedDocument<IUserFields>, UserModel, UserMethods, {}>({
  username: {
    type: String,
    required: true,
    unique: true,
    validate: {
      validator: async function (value: string): Promise<boolean> {
        ...
        const user: HydratedDocument<IUserFields> | null = await User.findOne({ username: value });
        return !user;
      },
